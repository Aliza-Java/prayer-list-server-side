...
public class AdminService {
	...
	public void createDavener(Davener davener) {
	...
	public void deleteSubmitter(long id) {
		submitterRepository.deleteById(id);
	}

	public Submitter getSubmitter(long id) throws ObjectNotFoundException {
		Optional<Submitter> optionalSubmitter = submitterRepository.findById(id);
		if (!optionalSubmitter.isPresent()) {
			throw new ObjectNotFoundException("Submitter with id " + id);
		}
		return optionalSubmitter.get();
	}

	public void deleteDavenfor(long id) {
		davenforRepository.deleteById(id);
	}

	@Transactional
	public Davenfor getDavenfor(long id) throws ObjectNotFoundException {
		Optional<Davenfor> optionalDavenfor = davenforRepository.findById(id);
		if (!optionalDavenfor.isPresent()) {
			throw new ObjectNotFoundException("Davenfor with id " + id);
		}
		return optionalDavenfor.get();
	}

	public void updateSubmitterName(String newName) {
		Optional<Submitter> optionalSubmitter = submitterRepository.findById((long) 7);
		if (optionalSubmitter.isPresent()) {
			Submitter updatedSubmitter = optionalSubmitter.get();
			updatedSubmitter.setName(newName);
			submitterRepository.save(updatedSubmitter);
		}

	}

	public void saveUpdatedDavenfor() {
		Optional<Davenfor> optionalDavenfor = davenforRepository.findById((long) 16);
		if (optionalDavenfor.isPresent()) {
	...
	public void saveUpdatedCategory() {
		Optional<Category> optionalCategory = categoryRepository.findById((long) 8);
		if (optionalCategory.isPresent()) {
